...
if (Sys.info()['sysname'] == 'Darwin') {
  chooseDirectory = function(default = NA, caption = NA) {
    command = 'osascript'
...
chooseDirectory<-function(default){
  root = tktoplevel("width" = 1, "height" = 1)
  tkraise(root)
  dir <- tclvalue(tkchooseDirectory( initialdir=default, parent= root))
  if (Sys.info()['sysname']=="Windows")
  {
    dir<-gsub(x = dir,pattern = "/", replacement = "\\\\\\")
  }
  tkwm.withdraw(root)
  return(dir)
}
